...
class TestFontFlags:
    def test_prefers_lato(self):
        with patch(
            "builder_profile.report._fc_families",
            return_value={"lato", "dejavu sans mono"},
        ):
            flags = _font_flags()
        assert "mainfont=Lato" in flags
        assert "sansfont=Lato" in flags
        assert "monofont=DejaVu Sans Mono" in flags

    def test_falls_back_when_no_lato(self):
        with patch("builder_profile.report._fc_families", return_value={"dejavu sans"}):
            flags = _font_flags()
        assert "mainfont=DejaVu Sans" in flags

    def test_no_fonts_means_no_flags(self):
        with patch("builder_profile.report._fc_families", return_value=set()):
            assert _font_flags() == []

    def test_fc_list_absent_returns_empty(self):
        with patch("shutil.which", return_value=None):
            assert _fc_families() == set()
...
class TestRenderPdf:
    def test_success_returns_true(self, tmp_path):
        ...
        with (
            patch("builder_profile.report._font_flags", return_value=["-V", "mainfont=Lato"]),
        ...
        assert "mainfont=Lato" in call_args  # font flags are incorporated
    ...
    def test_nonzero_returncode_returns_false(self, tmp_path):
        ...
        with (
            patch("builder_profile.report._font_flags", return_value=[]),
